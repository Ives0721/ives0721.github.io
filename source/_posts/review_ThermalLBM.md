...
因此可以将总能的分布函数定义为$h = \frac{1}{2}\xi^{2}f$, 则$\rho E = \int_{}^{}hd\mathbf{\xi}$。
...
Guo等 [^10] 将平衡态使用Hermite展开对${\overline{f}}^{(eq)}$和${\overline{h}}^{(eq)}$进行离散, 从而使低阶速度矩不会因高阶项的舍去而产生影响。由于涉及局部温度$T$, 因此展开过程是基于参考温度$T_{0}$进行的。离散平衡态为：
